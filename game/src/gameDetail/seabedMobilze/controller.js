var SeabedMobilze = cc.Layer.extend({
	_fish_number:null,
	_goal_color:null,
	_goal_icon:null,
	_goal_number:null,
    ...
    ctor: function (args) {
        ...
        this._view = new SeabedMobilzeView(args.bg_color);
    ...
    runOneGame:function(level){
        ...
        this._way = new Array();
        this._speedArray = new Array();
        this.chooseLevelSpeed(level);
        this.subTime = this.shadeMinTime();
    ...
    chooseLevelSpeed: function (level) {
        var speedIndex;
        if (level > -1 && level < 15) {
            speedIndex = 0;
        }else if (level > 14 && level < 23) {
            speedIndex = 1;
        ...
        for (var i = 0; i < 3; i++) {
            this._speedArray[i] = move_speed[speedIndex][i];
        };
    },

    shadeMinTime: function () {
        var obstacle_show = cc.winSize.width - this.unit.channal12_width;
        var minTime = obstacle_show / this._speedArray[2];
        return minTime;
    },
    ...
    showFish: function (cPosY, fishCount, hCount) {
        ...
        this._col = 0;
        this._remainCount = 0;
        var speed_index = util.getRandom(1, 3) - 1;
        this._speed = this._speedArray[speed_index];
    ...
    setFishPos: function (cPosY, fCount, hCount) {
		...
		for (var j = 0; j < colCount; j++) {
            ...
            // this.numberNature.fishRes = this.getFishType(this._fish_number);
            var fishThings = this.getFishType(this._fish_number);
            this.numberNature.fishRes = fishThings.fish_res;
            this.numberNature.fishId = fishThings.fish_typeId;
            this.numberNature.fishSpeed = this._speed;

			var fish = new SeabedNumber(this.numberNature);
    ...
    getFishType: function (fishCount) {
    	...
    	return {fish_res: fish_res, fish_typeId: fish_typeId};
    },
    ...
    moveFishType1: function (node) {
        ...
        if (node._swim_direction == 2) {
            distance = node_posX - 0 + node.width;
        }
    ...
    moveFishType2: function (node) {
    	...
    	var node_posX = node.x;
    	var fish_size = node.getContentSize();
        ...
        var fw_times = swim_time1 + 0.5 + swim_time2;
    ...
    showObstacle:function () {
    	var size = cc.winSize;
        this._view.obstacle_left.setPosition(0, 150);
        this._view.obstacle_right.setPosition(size.width, 150);
    ...
    setStopAction: function () {
        ...
        this._view.obstacle_left.stopAllActions();
        this._view.obstacle_right.stopAllActions();
        this._view.fishbowl.stopAllActions();
        this.stopAllActions();
    }, 
    ...
    handleRestart:function(self){
        ...
        self.setGamePauseResume(false);
        self.setStopAction();
        self._view._fishLayer.removeAllChildren(true);        
        self._viewGameSetting.removeFromParent(true); 
        // self.stopAllActions();
        self._stopSchedule = false;
