var WorldView = cc.LayerColor.extend({
    _touchCount:null,
    _gn_width:null,
    ...
    showTouch: function (touchPos, score) {
        var size = cc.winSize;
        var scale = size.width / 750;
        ...
        var addOne = new cc.LabelTTF(scoreStr, "Arial", 45);
        addOne.setAnchorPoint(cc.p(0.5, 0.5));
        addOne.attr({
            x: touchPos.x,
            y: touchPos.y,
            scale:scale
    ...
    touchFly: function (node) {
        ...
        var move = cc.MoveBy.create(move_time, cc.p(0, 150));
    ...
    showScore: function (touchPoint, score) {
        this.showTouch(touchPoint, score);
        this._touchCount = this._touchCount + score;
        if (this._touchCount < 0) {
            this._touchCount = 0;
        }
        this.getTouchCount(this._touchCount);
        this._handle(this._delelgate);
    },
    ...
    setAccountView: function (gfRes) {
        ...
        this.touch_count.setString(this._touchCount);  
    ...
    addGuideHand: function (pos) {
        ...
        this.hand.attr({
            x:pos.x + 100,
            y:pos.y - 100 - this.hand.height*0.5,
            scale:scale
        });
        this.addChild(this.hand, 3);

        this.addHandAction(pos);
    },

    addHandAction: function (pos) {
        var size = cc.winSize;
        var scale = size.width / 750;
        ...
        var self = this;
        var handTouch = function () {
            self.handClick(pos);
        }
        this.scheduleOnce(handTouch,0.6);
    },

    handClick: function (pos) {
        var size = cc.winSize;
        var scale = size.width / 750;
